...
public class AttendanceLogScheduler {
    ...
    public void attendanceLogSchedule() {
        ...
        attendanceLogRepository.findAllForSchedule().forEach(al -> {
            Schedule schedule = scheduleRepository.findByAttendanceLog(al.getMember(), al.getPatientId(), al.getCheckIn().toLocalDate()).orElseThrow(() -> new CustomException(HttpStatusCode.NOT_FOUND, "로그 기록 상 일치하는 것이 되는 스케줄이 없다."));
